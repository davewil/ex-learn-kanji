defmodule KumaSanKanji.SRS.UserKanjiProgress do
  ...
  attributes do
    ...
    attribute :interval, :integer, default: 1, allow_nil?: false  # Days between reviews
    attribute :ease_factor, :decimal, default: Decimal.new("2.5"), allow_nil?: false  # SM-2 ease factor
    attribute :repetitions, :integer, default: 0, allow_nil?: false  # Successful repetitions count
    attribute :last_result, :atom, allow_nil?: true  # :correct, :incorrect, :skip
  ...
  def update_srs_state(changeset) do
    ...
    current_ease_factor = Ash.Changeset.get_attribute(changeset, :ease_factor) || Decimal.new("2.5")
    ...
    {new_interval, new_ease_factor, new_repetitions, new_correct_count} = 
      case result do
        :correct ->
          ...
          {new_interval, new_ease_factor} = calculate_sm2_interval(
            current_interval, 
            current_ease_factor, 
            new_repetitions,
            5  # Quality of response (5 = perfect)
          )
        ...
        :skip ->
          ...
          {max(1, div(current_interval, 2)), current_ease_factor, current_repetitions, current_correct_reviews}
      end
    
    # Calculate next review date
    next_review_date = DateTime.add(current_time, new_interval * 24 * 60 * 60, :second)
  ...
  def calculate_sm2_interval(interval, ease_factor, repetitions, quality) do
    ...
    new_ef = max(new_ef, 1.3)  # Minimum ease factor is 1.3
    
    # Calculate new interval
    new_interval = case repetitions do
      1 -> 1
      2 -> 6
