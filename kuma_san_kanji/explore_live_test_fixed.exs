defmodule KumaSanKanjiWeb.ExploreLiveTest do
  ...
  describe "ExploreLive functionality" do
    setup do
      ...
      {:ok, meaning1} =
        Meaning.create(%{value: "test", language: "en", is_primary: true, kanji_id: k1.id})

      {:ok, pron1} =
        Pronunciation.create(%{value: "し", type: "on", romaji: "shi", kanji_id: k1.id})
      ...
      # Small delay to ensure distinct inserted_at
      Process.sleep(5)

      {:ok, k2} = Kanji.create(%{character: "験", grade: 1, stroke_count: 18, jlpt_level: 2})

      {:ok, meaning2} =
        Meaning.create(%{value: "experience", language: "en", is_primary: true, kanji_id: k2.id})

      {:ok, pron2} =
        Pronunciation.create(%{value: "けん", type: "on", romaji: "ken", kanji_id: k2.id})
      ...
      {:ok, k1_with_rels} =
        Kanji.get_by_id(k1.id, load: [:meanings, :pronunciations, :example_sentences])

      {:ok, k2_with_rels} =
        Kanji.get_by_id(k2.id, load: [:meanings, :pronunciations, :example_sentences])
